...
class Solver(object):
    def __init__(self, loaders, model, optimizer, config, args):
        ...
        augments = [
            augment.Shift(
                shift=int(config.data.samplerate * config.data.shift),
    ...
    def _serialize(self, epoch, steps=0):
        ...
        if steps:
            checkpoint_with_steps = Path(self.checkpoint_file).with_name(
                f"checkpoint_{epoch + 1}_{steps}.th"
            )
    ...
    def _reset(self):
        ...
        if self.checkpoint_file.exists():
            ...
            package = torch.load(
                self.checkpoint_file, map_location=self.accelerator.device
            )
        ...
        for epoch in range(self.epoch + 1, self.config.epochs):
            ...
            for param_group in self.optimizer.param_groups:
                param_group["lr"] = self.config.optim.lr * (
                    self.config.optim.decay_rate
                    ** ((epoch) // self.config.optim.decay_step)
                )
                logger.info(
                    f"Learning rate adjusted to {self.optimizer.param_groups[0]['lr']}"
                )
            ...
            logger.info(f"Valid Summary | Epoch {epoch + 1} | {_summary(formatted)}")
